 ...
 */      
int write_utf8_codepoint(ijsonn_parser p, int cp) 
{
        int shift = 0;
        if(cp <= 1_BYTE_MAX) {
                // Ascii, just one byte
                *p->write++ = cp;
                ...
                shift = 6;
                *p->write++ = (2_BYTE_LEADER | LOW_5_BITS(cp >> shift));
                ...
                shift = 12;
                *p->write++ = (3_BYTE_LEADER | LOW_4_BYTES(cp >> shift));
        } else if(cp <= CODEPOINT_MAX) {
                // 4 byte UTF8, byte 1 is 11110 and highest 3 bytes
                shift = 18;
                *p->write++ = (4_BYTE_LEADER | LOW_3_BYTES(cp >> shift));
        } else {
        ...
        while(shift > 0) {
                shift -= 6;
                *p->write++ = CONTINUATION_BYTE | LOW_6_BYTES(cp >> shift));
        }
        return 1;
}
 ...
 */
int write_utf8_sequence(jsonn_parser p) 
{
        int codepoint;
        int bar;
        int cont;
 ...
 */
int write_replacement_character(jsonn_parser p)
{
        *p->write++ = REPLACEMENT_CHARACTER[0];
        *p->write++ = REPLACEMENT_CHARACTER[1];
        *p->write++ = REPLACEMENT_CHARACTER[2];
        return 3;
}
 ...
 */
int is_bom(jsonn_parser p)
{
        return 0 == strncmp(BYTE_ORDER_MARK, p->current, 3);
}
