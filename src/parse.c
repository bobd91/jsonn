...
static jsonn_type begin_object(jsonn_parser p) {
  p->current++;
  if(push_next(p, PARSE_OBJECT_MEMBER_OPTIONAL) < 0) return JSONN_UNEXPECTED;
  return JSONN_BEGIN_OBJECT;
}

static jsonn_type end_object(jsonn_parser p) {
  p->current++;
  if(pop_next(p) < 0) return JSONN_UNEXPECTED;
  return JSONN_END_OBJECT;
}

static jsonn_type begin_array(jsonn_parser p) {
  p->current++;
  if(push_next(p, PARSE_ARRAY_VALUE_OPTIONAL) < 0) return JSONN_UNEXPECTED;
...
static jsonn_type parse_literal(jsonn_parser p, char *literal, jsonn_type type) {
...
static parse_next consume_array_value_separator(jsonn_parser p) {
  if(p->current < p->last) {
    ...
    case ',':
      p->current++;
      return (p->flags && FLAG_ALLOW_TRAILING_COMMAS) ? PARSE_ARRAY_VALUE_OPTIONAL : PARSE_ARRAY_VALUE;
    case ']':
...
static parse_next consume_object_member_separator(jsonn_parser p) {
  if(p->current < p->last) {
    ...
    case ',':
      p->current++;
      return (p->flags && FLAG_ALLOW_TRAILING_COMMAS) ? PARSE_OBJECT_MEMBER_OPTIONAL : PARSE_OBJECT_MEMBER;
    case '}':
      // not separator so don't consume
...
static int is_eof(jsonn_type type) {
  return type == JSONN_EOF;
}
